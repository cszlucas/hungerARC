...
const Expense = () => {
  ...
  const handleSave = async () => {
    if (eventEditMode.id === "new") {
      ...
      if (!user.guest) {
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/scenario/${editMode}/expenseEvent`, formValues);
      ...
      if (!user.guest) await axios.post(`${process.env.REACT_APP_API_URL}/updateExpense/${eventEditMode.id}`, formValues);
        ...
        <Box sx={rowBoxStyles}>
          ...
          <Box sx={{ flex: 1, display: "flex", flexDirection: "column", width: 300 }}>
            ...
            <Stack direction="row" alignItems="center" spacing={2} sx={{ marginTop: 4, mb: 1 }}>
              ...
              <Checkbox checked={formValues.inflationAdjustment} onChange={(value) => handleInputChange("inflationAdjustment", value.target.checked)} />
            </Stack>
