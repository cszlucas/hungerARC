...
const Income = () => {
  ...
  useEffect(() => {
    ...
    const expression =
      ...
      (currScenario.filingStatus !== "married" || checkValidNum(formValues.userPercentage)) &&
      (formValues.annualChange.distribution !== "none" || checkValidNum(formValues.annualChange.amount)) &&
      (formValues.annualChange.distribution !== "normal" || (checkValidNum(formValues.annualChange.mean) && checkValidNum(formValues.annualChange.stdDev))) &&
      (formValues.annualChange.distribution !== "uniform" ||
        (checkValidNum(formValues.annualChange.min) && checkValidNum(formValues.annualChange.max) && formValues.annualChange.min <= formValues.annualChange.max));
  ...
  const handleSave = async () => {
    try {
      if (eventEditMode.id === "new") {
        ...
        if (!user.guest) {
          const response = await axios.post(`${process.env.REACT_APP_API_URL}/scenario/${editMode}/incomeEvent`, formValues);
        ...
        if (!user.guest) await axios.post(`${process.env.REACT_APP_API_URL}/updateIncome/${eventEditMode.id}`, formValues);
        setCurrIncome((prev) => {
        ...
        <Box sx={rowBoxStyles}>
          ...
          <Box sx={{ flex: 1, display: "flex", flexDirection: "column", width: 300 }}>
            ...
            <Stack direction="row" alignItems="center" spacing={2} sx={{ marginTop: 4, mb: 2 }}>
              ...
              <Checkbox checked={formValues.inflationAdjustment || false} onChange={(e) => handleInputChange("inflationAdjustment", e.target.checked)} />
            </Stack>
            ...
            <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 5 }}>
              ...
              <Checkbox checked={formValues.isSocialSecurity || false} onChange={(e) => handleInputChange("isSocialSecurity", e.target.checked)} />
            </Stack>
